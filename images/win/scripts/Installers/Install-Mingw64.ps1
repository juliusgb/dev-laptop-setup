...
if (Test-IsWin22 -or Test-IsWinHome) {
    ...
    $("mingw32", "mingw64") | ForEach-Object {
        ...
        Expand-7ZipArchive -Path $packagePath -DestinationPath "C:\opt"
        ...
        $path = "C:\opt\$_\bin\mingw32-make.exe" | Get-Item
        Copy-Item -Path $path -Destination (Join-Path $path.Directory 'make.exe')
    }

    Add-MachinePathItem "C:\opt\mingw64\bin"
}
